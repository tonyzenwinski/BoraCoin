...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBoraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBoraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBoraAmount = strAnonymizeBoraAmount.remove(strAnonymizeBoraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBoraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBoraAmount*COIN) nMaxToAnonymize = nAnonymizeBoraAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBoraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBoraAmount));
        strAnonymizeBoraAmount = strAnonymizeBoraAmount.remove(strAnonymizeBoraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBoraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
