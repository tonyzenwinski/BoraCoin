...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBoraAmount"))
        settings.setValue("nAnonymizeBoraAmount", 1000);
    nAnonymizeBoraAmount = settings.value("nAnonymizeBoraAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBoraAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBoraAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBoraAmount:
            return QVariant(nAnonymizeBoraAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBoraAmount:
            nAnonymizeBoraAmount = value.toInt();
            settings.setValue("nAnonymizeBoraAmount", nAnonymizeBoraAmount);
            emit AnonymizeBoraAmountChanged(nAnonymizeBoraAmount);
            break;
        default:
            break;
        }
