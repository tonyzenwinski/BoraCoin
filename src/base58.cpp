...
namespace {
    class CBoraAddressVisitor : public boost::static_visitor<bool> {
    private:
        CBoraAddress *addr;
    public:
        CBoraAddressVisitor(CBoraAddress *addrIn) : addr(addrIn) { }
...
bool CBoraAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CBoraAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CBoraAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CBoraAddressVisitor(this), dest);
}

bool CBoraAddress::IsValid() const {
...
CTxDestination CBoraAddress::Get() const {
    if (!IsValid())
...
bool CBoraAddress::GetKeyID(CKeyID &keyID) const {
...
bool CBoraAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CBoraSecret::SetKey(const CKey& vchSecret) {
...
CKey CBoraSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CBoraSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CBoraSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBoraSecret::SetString(const std::string& strSecret) {
